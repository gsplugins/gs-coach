...
import { CoachesEdit } from './edit';

/**
 * Localized builder data (from PackageBuildManager data_app_window).
 */
const getBuilderData = () => window?.GsCoachDiviVbData || {};
...
const registerModuleIcon = () => {
  addFilter('divi.iconLibrary.icon.map', 'gscoach.coaches', (icons) => {
    const icon = {
      name: 'gscoach/module-coach',
      viewBox: '0 0 16 16',
      component: () => (
        // Scale brand coach glyph (81×80) into Divi's 16×16 module-icon viewport.
 ...
 */
const registerCoachesModule = () => {
  const { metadata: moduleMetadata, ...moduleDefinition } = coachesModule;
